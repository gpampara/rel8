{-# language TypeInType, KindSignatures, DataKinds, DeriveGeneric, FunctionalDependencies, TypeFamilies, RankNTypes, FlexibleContexts, NoMonomorphismRestriction, FlexibleInstances, GeneralizedNewtypeDeriving, TypeApplications #-}
...
class Table (expr :: Type) (t :: Type) | t -> expr where
  type RowF t :: Type -> Type

  toRow :: t -> RowF t expr
  fromRow :: RowF t expr -> t

class BaseTable t where
  tableName :: Tagged t String

queryTable :: forall t m. (Table (Limit (ExprT m)) t) => m t
...
select :: (Table (Limit (ExprT Q)) expr) => Q expr -> IO a
select = undefined
...
aggregate
  :: (Table (Limit (ExprT m)) aggregated)
...
instance Table (Limit expr) (T expr) where

instance Table (Limit (ExprT m :: Type -> Type)) (ExprT m a)
